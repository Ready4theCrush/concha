...
class ProfitMaximizer(BaseEstimator, RegressorMixin, DeepLearningEstimator):
    ...
    def fit(self, X, y=None, validation_data=None):
        ...
        regr_loss = self.create_maximize_profit_regression_loss_function(self.marginal_cost, self.unit_sale_price)
        self.regression_model_.compile(loss=regr_loss, optimizer='adam')
        ...
        optimal_rounding_fn = self.make_rounding_direction_function(self.batch_size, self.batch_cost, self.unit_sale_price)
        disc_target = optimal_rounding_fn(y_original, regr_prediction)
    ...
    def predict(self, X):
        ...
        X_conv = self.X_converter_.transform(X)
...
class QuantileRegressor(BaseEstimator, RegressorMixin, DeepLearningEstimator):
    ...
    def fit(self, X, y=None, validation_data=None):
        ...
        regr_loss = self.create_quantile_regression_loss_function(self.demand_quantile)
        self.regression_model_.compile(loss=regr_loss, optimizer='adam')
        callback = EarlyStopping(monitor=earlystopping_monitor, patience=7)
        history = self.regression_model_.fit(
            X_conv,
