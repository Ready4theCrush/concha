...
class Product():
    ...
    def score_model(self, target='estimated_demand', true_demand='estimated_demand', cv=5):
        ...
        X = records.drop(columns=['date', 'estimated_demand', 'measured_demand', 'demand'], errors='ignore')
...
#         splitter = ShuffleSplit(n_splits=cv, test_size=1/cv)
        ...
        score['profit_margin'] = np.sum(score['profit'])*100.0/np.sum(score['revenue'])
    ...
    def predict_production(self, records):
        ...
        """
        X = records.copy() 
        X = X.drop(columns=['date', 'estimated_demand', 'measured_demand', 'demand'], errors='ignore')
    ...
    def estimate_demand(self, num_quantiles=24):
        ...
        by_max_minute['qnt'] = pd.qcut(by_max_minute['minute'], 5, labels=False, duplicates='drop')
        ...
        ser, bins = pd.qcut(trans[trans['date'].isin(train_dates)]['minute'], num_quantiles, retbins=True, labels=False, duplicates='drop')
        ...
        trans['quantile'] = pd.cut(trans['minute'], bins=bins, labels=False, include_lowest=True)
    
        # Get the number of sales in each quantile by date.
        sales_by_quantile = trans.groupby(['product', 'date', 'quantile'])['quantity'].sum().reset_index()
        sales_by_quantile['cumulative_sales'] = sales_by_quantile.groupby(['product', 'date'])['quantity'].transform(pd.Series.cumsum)
        ...
        for date, dt_sales in sales_by_quantile.groupby('date'):
            record = {
                'date': date,
                'max_quantile': dt_sales['quantile'].max(), # max quantile transaction time.
                'is_all_demand': dt_sales['quantile'].max() == num_quantiles-1, # if last transaction happened at end of day.
                'max_known_quantile': dt_sales[dt_sales['quantile'] < dt_sales['quantile'].max()]['quantile'].max(),
                # last quantile where we know demand was not cutoff by stockout.
                'known_demand': dt_sales[dt_sales['quantile'] < dt_sales['quantile'].max()]['cumulative_sales'].max(), # the amount at that quantile.
        ...
        for quantile in range(1,num_quantiles-1):
            limited_by_quantile = training_set[training_set['quantile'] < quantile]     #train_model_dates[train_model_dates['quantile'] <= quantile]
        ...
        by_date['estimated_demand'] = by_date.apply(lambda row:
            row['total_demand'] if row['is_all_demand']
            else quantile_models[row['max_known_quantile']].predict(np.array([[row['known_demand']]]))[0]
        , axis=1)
        by_date['estimated_demand'] = by_date['estimated_demand'].round()
        by_date['measured_demand'] = by_date['total_demand']
        self.daily_demand_history = by_date[['date', 'estimated_demand', 'measured_demand']]
